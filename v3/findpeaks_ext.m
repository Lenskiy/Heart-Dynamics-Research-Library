function [Ypk,Xpk,Wpk,Ppk] = findpeaks_ext(Yin,varargin)
...
[y,yIsRow,x,xIsRow,minH,minP,minW,maxW,minD,minT,ncT,maxN,sortDir,annotate,refW] ...
...
%Add new call
iPk = removePeaksBelowNonConstantThreshold(y,iPk,ncT);
...
%--------------------------------------------------------------------------
function [y,yIsRow,x,xIsRow,Ph,Pp,Wmin,Wmax,Pd,Th,NCTh,NpOut,Str,Ann,Ref] = parse_inputs(Yin,varargin)
...
% Added new parameter
defaultNonConstantThreshold = -inf * ones(length(y), 1);
...
if coder.target('MATLAB')
    ...
    % Added new parameter
    addParameter(p,'NonConstantThreshold',defaultNonConstantThreshold);
    ...
    %Add new parameter
    NCTh = p.Results.NonConstantThreshold;
else
    ...
    % Adde new parameter
    NCTh = eml_get_parameter_value(pstruct.NonConstantThreshold,NonConstantThreshold,varargin{startArg:end});
end
...
%added
validateattributes(NCTh,{'numeric'},{'real','nonempty'},'findpeaks','NonConstantThreshold');
...
%--------------------------------------------------------------------------
function iPk = removePeaksBelowNonConstantThreshold(Y,iPk,NCTh)

iPk = iPk(Y(iPk) >= NCTh(iPk));

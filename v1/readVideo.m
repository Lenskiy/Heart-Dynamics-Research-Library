function [video, frameRate] = readVideo(file, reduction_coef)
    ...
    while hasFrame(xyloObj)
        ...
        video{1} = im(1:reduction_coef:end, 1:reduction_coef:end, 1);
        video{2} = im(1:reduction_coef:end, 1:reduction_coef:end, 2);
        video{3} = im(1:reduction_coef:end, 1:reduction_coef:end, 3);
        k = k + 1;

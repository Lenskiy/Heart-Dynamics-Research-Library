function cleanPPG = cleanupPPG(red_ch, blue_ch)

    b = fir1(128,[0.05 0.4]);
    red_ch_filtered = filtfilt(b,1,red_ch); % filter
    red_ch_filtered = (red_ch_filtered - mean(red_ch_filtered))/std(red_ch_filtered); % normalize
